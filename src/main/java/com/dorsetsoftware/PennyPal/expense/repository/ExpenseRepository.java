...
public interface ExpenseRepository extends JpaRepository<Expense, Long> {
    ...
    @Query("""
            SELECT YEAR(e.date) AS year, MONTH(e.date) AS month, SUM(e.amount) AS total
                    ...
                    JOIN e.category c
                    LEFT JOIN c.parent parent
                            WHERE (TRUE = :#{#startDate == null} OR e.date >= :startDate)
                            AND COALESCE(parent.type, c.type) = 'EXPENSE'
                            AND COALESCE(parent.id, c.id) <> 51
                    GROUP BY YEAR(e.date), MONTH(e.date)
                    ORDER BY year, month
            """)
    List<Object[]> getMonthlyTotalsSince(@Param("startDate") LocalDate startDate);
    ...
    @Query("""
            SELECT YEAR(e.date) AS year, MONTH(e.date) AS month, SUM(e.amount) AS total
                    ...
                    LEFT JOIN c.parent parent
                            WHERE (TRUE = :#{#startDate == null} OR e.date >= :startDate)
                            AND COALESCE(parent.type, c.type) = 'INCOME'
                    GROUP BY YEAR(e.date), MONTH(e.date)
                    ORDER BY year, month
            """)
    List<Object[]> getMonthlyIncomeSince(@Param("startDate") LocalDate startDate);

    @Query("""
                SELECT COALESCE(SUM(e.amount), 0)
                FROM Expense e
                WHERE e.user.username = :username
                  AND (:categoryId IS NULL OR e.category.id = :categoryId OR e.category.parent.id = :categoryId)
                  AND e.category.type = 'EXPENSE'
                  AND COALESCE(e.category.parent.id, e.category.id) <> 51
                  AND (TRUE = :#{#startDate == null} OR e.date >= :startDate)
            """)
    BigDecimal sumByCategoryIdAndUser(
